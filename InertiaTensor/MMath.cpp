...
{
	Matrix3 inverseM = m;
	float determinant = MMath::determinant(m);

	// store the transpose of the matrix
	Matrix3 mT = transpose(inverseM);

	// the Adjugate Matrix3 of Matrix3 m
	// calculated by taking all of the minors of the transposed matrix of m: mT
	Matrix3 mAdj((mT[4] * mT[8] - mT[5] * mT[7]), (mT[3] * mT[8] - mT[5] * mT[6]), (mT[3] * mT[7] - mT[4] * mT[6]),
	...
	// multiplying each spot in the adjucate matrix by the appropriate sign
	mAdj[0] *= 1.0f;
	mAdj[1] *= -1.0f;
	mAdj[2] *= 1.0f;
	mAdj[3] *= -1.0f;
	...
	// the final calculation for the inverse matrix
	inverseM = mAdj * (1.0f / determinant);

	return inverseM;
}

// calculate the determinant of a 3x3 matrix
float MMath::determinant(const Matrix3& m)
{
	float determinant;

	// these are the cofactors of the given Matrix, the sum of these 3 values is the determinant
	float minorA11 = (m[0] * 1.0f) * ((m[4] * m[8]) - (m[5] * m[7]));
	float minorA12 = (m[3] * -1.0f) * ((m[1] * m[8]) - (m[2] * m[7]));
	float minorA13 = (m[6] * 1.0f) * ((m[1] * m[5]) - (m[4] * m[2]));

	determinant = minorA11 + minorA12 + minorA13;

#ifdef _DEBUG  // If in debug mode let's worry about divide by zero or nearly zero!!! 
	if (fabs(determinant) < VERY_SMALL) {
...
/// Didn't check it - Thank you Dr. Rob Robson for the fix 
/// Tested 2016
